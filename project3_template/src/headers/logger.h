/*
    Here is a tool to help you store running results to file system for further
   analysis and reproducing. API: Create a logger: Logger l = Logger(const char*
   version, int n_body_, int x_bound_, int y_bound_); Save a new frame:
...
Logger::Logger(const char* version_, int n_body_, int x_bound_, int y_bound_) {
    ...
    path = root_path + version + "_" + std::to_string(n_body) + "_" +
           start_time + "/";
    std::cout << path << std::endl;

    if (is_exist(root_path.c_str()) == 0) {
        mkdir(root_path.c_str(),
              S_IRUSR | S_IWUSR | S_IXUSR | S_IRWXG | S_IRWXO);
    }

    if (is_exist(path.c_str()) == 0) {
        int isCreate = mkdir(path.c_str(),
                             S_IRUSR | S_IWUSR | S_IXUSR | S_IRWXG | S_IRWXO);
        if (!isCreate) {
...
void Logger::save_frame(double* x, double* y) {
    ...
    for (int i = 0; i < n_body; i++) {
        f << std::to_string(x[i]) << std::endl
          << std::to_string(y[i]) << std::endl;
    }
    f.close();
    update_metadata();
    return;
